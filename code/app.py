...
@app.route("/")
def index():
    return render_template(r'Model/templates/home.html')
...
def result():
   ...
   # print(item_fat_content)
    ...
    le=joblib.load(r'F:\ML Projects\Store sales prediction\Model\le.sav')
...
# Lets put all in the list
    ...
    sc = joblib.load(r'F:\ML Projects\Store sales prediction\Model\sc.sav')
    inputs_std= sc.transform(inputs)

    # Lets apply prediction

    model=joblib.load(r'F:\ML Projects\Store sales prediction\Model\gbr.sav')

    prediction=model.predict(inputs_std)
    prediction=prediction.tolist()


    return jsonify({'Sales prediction': prediction})
